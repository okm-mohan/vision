...
export function CameraFeed({camera, large=false}: {camera:Camera; large?:boolean}) {
  return <article className={`camera-feed ${camera.tone} ${large?'large':''}`}><video className="cctv-video" autoPlay loop muted playsInline src={`/sample_media/${camera.tone}.mp4`} />
